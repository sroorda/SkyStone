...
*/
        log("[moveWithEncoder] speed",""+speed);
        runMotorsUntilPositionRampSpeed(speed, 0, 0, fullSpeed, roundedDistance, STATE_MOVE);
    }

    /*public void moveWithEncoderDecel(double distance, double speed, boolean fullSpeed) {
        double percentToTarget = 1;
        ...
        percentToTarget = reachedDistance(distance, backLeft, frontLeft, backRight, frontRight);
        runMotorsUntilPositionRampSpeed(speed, 0, 0, fullSpeed, roundedDistance, STATE_MOVE);
    }*/
    ...
    protected void runMotorsUntilPositionRampSpeed(double drive, double strafe, double rotate, boolean fullSpeed, double targetDistance, int moveState) {
        ...
        while( opMode.opModeIsActive() && percentToTarget > 0) {
            double angle = getAngle();
            ...
            double speedFactor2 = speedFactor;

            if(percentToTarget<.5) {
                speedFactor2 = percentToTarget;
            }
            /*else if(percentToTarget < 0.1) {
                speedFactor2 = percentToTarget;
            }*/


            if(speedFactor2 < 0.3) {speedFactor2 = 0.3;}
            /*log("[runMotorsUntil] speedFactor",""+speedFactor);
            log("[runMotorsUntil] speedFactor2",""+speedFactor2);*/
            setMotorSpeeds(speedFactor2, flPower, blPower, frPower, brPower);
            //opMode.sleep(50);
            opMode.idle();
    ...
    public void setMotorSpeeds(double speedFactor, double flPower, double blPower, double frPower, double brPower) {
        ...
        log("power: ", String.format("sf %.5f bl %.5f fl %.5f br %.5f fr %.5f",speedFactor,blPower,flPower,brPower,frPower));
    ...
    private double reachedDistance(double targetDistance, DcMotor...ms) {
        ...
        double percentToTarget = (average + ENCODERS_CLOSE_ENOUGH) / Math.abs(targetDistance);
        percentToTarget = 1-percentToTarget;
        //log("[reachedDistance]", "" + average);
        //log("[reachedDistance]", "" + Math.abs(targetDistance));
        //log("[reachedDistance]", "" + percentToTarget);
        if(percentToTarget<0) {percentToTarget = 0;}
        return percentToTarget;
    }
